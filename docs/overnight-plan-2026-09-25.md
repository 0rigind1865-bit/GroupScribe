...
**目前狀態**：X2 可做子項完成；接著收尾 X2 的兩項待決（QR、舊資料）
**最後更新**：2026-09-26 18:15
**起始 commit**：`de67eae`
...
| 15:01|G8b|不寫程式，business-plan 2.1 的 G8 標為已滿足|schema.sql:88,140,156 三表 needs_confirmation default true；extract.ts:535,562,587 更新時強制 true；寫 false 的只有人工確認路徑（api/{tasks,notes,events}/update、api/batch） |
| 16:00|X1|用 groupscribe-preview 設定（ADMIN_PASSWORD=preview-only-local）登入本機看頁面，不讀真正密碼|R4 允許登入、R9 禁讀金鑰；登入路由確認不寫庫 |
| 16:02|X1|報帳 CSV 不含時間、付款方式、地點、照片連結欄（X2 才有這些欄位）|X1 表結構沒有這些欄；照片連結是 1 小時簽名網址，放進檔案會失效 |
| 16:10|X2|X2 所有新欄位集中在一支 migration 023（付款方式、來源、網頁照片路徑、座標、地名、自訂分類）|早上少貼幾段；某子項被跳過時多出的欄位無害 |
| 16:12|X2|expenseQuery 改 select('*')，寫入經 withV2Fallback|023 還沒跑時查詢與寫入都不失敗（規則 12） |
| 16:35|全域|dev server 開著時不跑 npm run build（兩者共用 .next 會互踩，頁面假性 500）；需要看畫面時 build 完再開 dev server|X2-3 提交後頁面 500，查 log 是 Cannot find module './5611.js'；清 .next 重開後 200 |
| 16:50|X2-4|分類管理用多行輸入框（一行一個＝增刪排序），不做逐列按鈕或拖曳|原生、零 JS；報帳模組因此有了「更多」頁，也順帶解決手機無法換身分（見發現清單第一條） |
| 17:30|X2-1|員工記帳做成獨立頁 /a/expense＋身分選單「我的報帳」，不塞進打卡頁的底部分頁|打卡頁導覽是五語系 i18n，加分頁要補五語；獨立頁改動最小。代價：此頁只有繁中 |
| 17:32|X2-1/2/7|三個子項在同一頁，合併一個 commit|拆開會產生中間狀態（有表單沒清單） |
