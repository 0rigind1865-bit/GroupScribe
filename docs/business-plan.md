...
| # | 要改什麼 | 檔案／表 | 人日 |
|---|---|---|---|
| A1 | ✅ **已完成 2026-09-23**（middleware 對 gs_liff 全放行，模組開關由兩個內殼各自驗；面向切換器依 modules 給入口）。原規劃：群組助理開放給 org_members：`visibleModules` 對 org 成員依 `org_settings.modules` 回模組（src/org/modules.ts:19-25）；middleware 拿掉「gs_liff 只放行 /attend」（src/middleware.ts:103-116）；`surfaces.ts:69` 讓 org_members 拿到「群組管理」面向 | src/org/modules.ts、src/middleware.ts、src/org/surfaces.ts | 1 |
| A2 | ✅ **已完成 2026-09-23**（13 支非白名單路由全數套上 `gsAccess()`；slug 取表單 `org` 或 Referer）。原規劃：19 支群組助理 API 補 org 綁定：共用 helper `gsAccess(form)` → `orgAdminAccess` → `{org, groupIds}`；帶 group_id 者走 `assertGroupInOrg`；以 id 操作者改 `.eq('id').in('group_id', groupIds)`；`reindex`（src/app/api/reindex/route.ts:13 目前刪全庫 embeddings）改逐 org | src/app/api/**（白名單：webhook/login/liff/auth/digest/attend） | 2 |
| A3 | ✅ **已完成 2026-09-23**（五處全綁本 org 群組；今天頁與收件匣的 `?group=` 不屬本 org 時視同未帶）。原規劃：5 個頁面跨群聚合補範圍：layout 徽章（(admin)/layout.tsx:28-33）、今天頁（(admin)/page.tsx:51-75）、收件匣 `filt()`（inbox/page.tsx:43-67）、groups 頁（groups/page.tsx:27-28）、files 頁（files/page.tsx:77）——先 `orgGroups()` 取 ids 再 `.in('group_id', ids)` | src/app/o/[org]/(admin)/* | 1 |
| A4 | ✅ **已完成 2026-09-23**（`tests/api-guard.test.ts`：白名單外必呼叫 gsAccess＋禁寫死舊路徑 redirect；prompt 來源規則未做）。原規劃：守門測試第四支：靜態掃描 `src/app/api/**/route.ts`，白名單外每支必 import `orgAdminAccess`／`gsAccess`；仿 tests/routes.test.ts:55-72。加規則：抽取／回答 prompt 的資料來源只能是本群查詢 | tests/api-guard.test.ts | 0.5 |
| G1 | ✅ **已完成 2026-09-25**（進群告知改存 org_settings、公司管理員可自改；認領成功即補發該公司告知＋consent_log——原本新群認領後從未被告知。模型／預算／webhook 心跳是全站共用金鑰與向量空間，刻意留在 app_settings 只給平台擁有者；設定頁平台數字只對平台擁有者顯示、抽取品質只算本公司群組）。原規劃：**app_settings id=1 全面遷 org_settings**：進群告知、模型、預算、`last_webhook_at` 改讀寫 org_settings（欄位已建但無人讀，src/org/orgs.ts:76-80）；改 src/core/ingest.ts:133-139、src/app/api/settings/route.ts:14-40、src/core/settings.ts:40-44、webhook 心跳、settings/page.tsx:39-58 | 上列 5 檔 | 1.5 |
| G2 | ✅ **已完成 2026-09-25**（`SESSION_SECRET`、`CRON_SECRET`，未設時暫退舊值；順手修 `/api/digest` 只檢查 cookie 名稱、假 cookie 即可觸發全站推播的漏洞）。原規劃：金鑰不再互借：LIFF session 簽章改獨立 `SESSION_SECRET`（src/core/liff.ts:25-27、src/middleware.ts:54-57 目前借 `LINE_CHANNEL_SECRET ?? ADMIN_PASSWORD`）；`/api/digest` 的 `?key` 改 `CRON_SECRET`（src/app/api/digest/route.ts:9-14） | 上列 3 檔 | 0.5 |
| A5 | ✅ **已完成 2026-09-23**（migration 016；認領頁 `/claim/[groupId]?t=`＋`/api/group/claim`；群組頁移轉下拉；`/api/digest` 順手退群）。原規劃：「未認領」歸戶：新 org `slug='unclaimed'`；`default_org_id()`（012_orgs.sql:23-26）改回 unclaimed。**未認領群不落地訊息、不抽取、不索引**，`@bot` 只 reply 固定文字「請管理員到後台認領此群」（reply 免費，src/core/ingest.ts:300-312）；未認領 7 天 bot 自動 leaveGroup。歸戶介面：平台擁有者在 `/o/main/groups` 每列「所屬組織」下拉。**認領／轉移一律 upsert groups 列（K2 修正 (2)）** | migrations/015、src/core/ingest.ts:110-118、extract.ts、indexer.ts、groups/page.tsx | 1.5 |
| A6 | ◐ **部分完成 2026-09-26**：`scripts/new-org.ts`、`scripts/seed-owner.ts` 已有；callback 自動種子已移除（main 已有 owner）；demo 拆 org 未做。原規劃：租戶一的資料手術：`scripts/split-main.ts` 一次性——demo 群搬到獨立 `demo` org；`ADMIN_LINE_USER_ID` 由 migration 種進 org_members(owner)，移除 callback 自動種子（src/app/api/auth/line/callback/route.ts:49-57）；`scripts/new-org.ts` 取代 README.md:260-262 的手動 SQL | scripts/、migrations/015、callback route | 1 |
| A11 | ✅ **已完成 2026-09-23**（migration 015；既有租戶回填 main＝兩者、其餘＝考勤）。模組開關 `org_settings.modules text[] default '{gs}'` | src/org/modules.ts、migration 015 | 0.5 |
| G3 | ✅ **已完成 2026-09-26**（migration 025：改用 groups 觸發器連動 org_id，涵蓋所有歸戶入口，未另寫 transfer_group()）。原規劃：**org_id 欄位先加、不寫 policy**：messages/events/tasks/notes/embeddings/media_assets/push_subscriptions 加 `org_id`，insert trigger 從 groups 帶入，backfill 一支 SQL；建 `transfer_group(gid, org)` 函式（以 groups.org_id 為唯一真相，K2 修正 (1)）。**注意**：此後轉移從單列 UPDATE 變成跨 6–7 表批次 UPDATE，大群可達數十萬列——仍非搬檔案，但要在背景跑 | migration 015 | 0.5 |
| G5 | ✅ **已完成 2026-09-26**（`createLineConnector(creds)`＋`envCreds`；liff.ts、middleware 未動）。原規劃：**（由階段二提前，K1 修正）** Connector 留門：`lineConnector` 由模組單例改 `lineConnector(creds)` 工廠＋`getConnector(channelId)`，只重構介面不做功能（12 處呼叫點：src/connectors/line.ts:7,18,157；src/core/liff.ts:22,26,35,100；auth/line 兩檔；ingest.ts:9-10,72-77；digest.ts:21-22,54；middleware.ts:57；三頁）。理由：形態 A 的條款風險（5.5(6)）與 Provider 規範無關，若 LINE 認定適用，形態 B 是唯一與條款結構一致的路徑，此時金鑰收成函式不能還沒做 | 上列 12 檔 | 1 |
| K3-1 | ✅ **已完成 2026-09-25**（migration 020）。**（K3 附帶）** `embeddings` 加 `(group_id)` btree 索引（schema.sql:58-59 目前無；match_embeddings 以 where group_id 後過濾 HNSW，schema.sql:120）——零風險單行 migration | migration 015 | 0.1 |
...
| # | 要改什麼 | 檔案／表 | 人日 |
|---|---|---|---|
| A7 | ✅ **已完成 2026-09-26**（`src/app/ui/onboarding-card.tsx`；有沒有群改看 groups 表；「已收到 N 則」；`LINE_BOT_BASIC_ID`）。原規劃：新租戶零資料上手卡：bot 加好友 QR／連結（env `LINE_BOT_BASIC_ID`）、「邀進群組」三步、「已收到 N 則，整理中」即時狀態；標明「未認證官方帳號（灰盾）」 | (admin)/page.tsx:141-148 | 1 |
| A8 | ◐ **部分完成 2026-09-24**：migration 017 加 `plan / max_groups / paid_until`，認領超額導 `/upgrade`；自助註冊 `/start`＋`/api/org/create`（free＝1 群）；2026-09-26 補 suspended 最小版（migration 026：停所有 AI、後台橫幅）；自動退群、刪資料未做。原規劃：方案欄位與硬上限：`org_settings` 加 `plan`、`status`、`max_groups`、`max_subscribers`、`monthly_extract_cap`、`paid_until`；超過拒絕歸戶並顯示升級文案；**降級語意**：Free 超保存期→只留索引；`suspended`（逾期 14 天）→停抽取與索引、bot 續留 30 天後 leaveGroup、資料再留 60 天後刪除，全程 1:1 私訊 org owner | migration 016、api/group/update、(admin)/layout.tsx | 1 |
| B5→ | ✅ **已完成 2026-09-25**（migration 018：org_usage＋monthly_ai_calls；六個 AI 入口 aiScope 門＋AsyncLocalStorage 記帳；方案頁與今天頁顯示）。原規劃 **（由階段二提前，K5 修正）** per-org 用量與預算煞車：`api_usage` 加 `org_id`＋呼叫類別（AsyncLocalStorage 帶 org；supabase/migrations/006_api_usage.sql:5-27、src/providers/gemini.ts:21-40）；超額（`org_settings.monthly_budget_usd` 或 `monthly_extract_cap`）先停抽取與 vision、保留落地與索引；平台級 `PLATFORM_MONTHLY_CAP_USD` 警報。理由：目前唯一煞車是 Gemini 回 429（gemini.ts:47），Free 永久免費在無封頂下等於開放式支出 | migration 016、gemini.ts、settings/page.tsx:53-59 | 2 |
| B7→ | ✅ **已完成 2026-09-26**（`src/core/schedule.ts`：安靜 45 秒、最多 3 分鐘；計時器在記憶體，重啟由下一則訊息接手）。原規劃：**（由階段二提前，K5 修正）** 抽取合批 debounce 30–60 秒：抽取 prompt 固定塞 profile＋既有項目 ≤60×3（MAX_EXISTING，src/core/extract.ts:11）＋否決清單＋前文 20 則，輸入與新訊息數幾乎無關；現況每批 webhook 事件就對群呼叫一次 extractGroup（src/app/api/webhook/line/route.ts:35-38），只有 running 鎖期間（extract.ts:182-195）才天然合批。把呼叫數壓到訊息「陣」數是最便宜的毛利槓桿。**Batch API 50% 折扣不適用**（目標 24 小時完成，https://ai.google.dev/gemini-api/docs/batch-api ） | src/core/worker.ts、extract.ts | 1 |
| A9 | ◐ **部分完成 2026-09-26**（`/api/health`＋Docker HEALTHCHECK；設定頁靜默 6 小時警示。Supabase 升 Pro、LINE redelivery、UptimeRobot 需本人操作；靜默私訊未做）。原規劃：營運底線：Supabase 升 Pro（US$25/月；Free 一週不活動暫停＝漏 webhook，https://supabase.com/pricing ，2026-09-23 查閱）；LINE console 開 redelivery；`GET /api/health`＋UptimeRobot；Dockerfile 加 HEALTHCHECK；`org_settings.last_webhook_at` 靜默 >6 小時私訊該 org 管理者 | Dockerfile、src/app/api/health/route.ts | 0.5 |
...
| A10 | ✅ **已完成 2026-09-24**（`/privacy`、`/terms`；2026-09-26 補設計夥伴合約草稿 docs/legal/design-partner-agreement.md）。原規劃：法務最低配：`/privacy`、`/terms`、設計夥伴一頁合約模板。隱私頁明列 Supabase 區域＋DPA（https://supabase.com/legal/customer-resources/data-processing-addendum ）、Gemini **一律付費 Tier**（免費層內容會被 Google 用於改進產品且人工可審閱，https://ai.google.dev/gemini-api/terms ，2026-09-23 查閱）、30 天刪除、72 小時通報 SOP。**對外文件不得宣稱「已符合 LINE 規範」，只寫「走 LINE 官方 Messaging API」（K1 修正）** | src/app/privacy、src/app/terms、docs/legal | 1 |
| P1 | ✅ **已完成 2026-09-26**（`src/app/manifest.ts`、192／512 icon、亮暗 themeColor）。原規劃：PWA 最低基礎：`public/manifest.json`＋icons＋`viewport`＋theme-color（目前全無：src/app/layout.tsx:4、無 public/） | src/app/layout.tsx、public/ | 0.5 |
| U2/U3/U12 | ✅ **已完成**（`?view=timeline`、選取模式、`/g` 文案）。原規劃：今天頁時間軸移出主畫面、待辦卡四鈕收斂、`/g` 空狀態文案修正（第 7 節） | (admin)/page.tsx:231-267、tasks/page.tsx:20-72、src/app/g/page.tsx:35 | 1.6 |
...
| 面向 | 決定 | 理由 |
|---|---|---|
| 資料模型 | 1:1 聊天室＝一個 `group_id = dm:<userId>` 的群，**零 schema** | 全部資料表以 group_id 為鍵（K2）；抽取／索引／LIFF 全部直接復用 |
| 歸戶 | 自動歸到該 userId 所屬的 org（`org_members`）；多個 org 時在 LIFF 選一次並記在 `groups.org_id`；不屬任何 org → 回 `/start` 引導 | 不需要認領流程：本人就是唯一成員 |
| 方案額度 | **不佔 `max_groups`**，但 AI 呼叫算在 org 的 `monthly_ai_calls` | 個人筆記是黏著度功能，不該吃掉 free 唯一的群；成本仍要有煞車 |
| 成員資格 | `isGroupMember(dm:<uid>, uid)` ＝ 只有本人；LIFF 群組清單顯示為「我的筆記」置頂 | 隱私邊界最單純的一種 |
| 說話規則 | 1:1 不是群組，沉默契約不適用：**句尾是問號或以「查／找」開頭才回答**，其餘靜默記錄 | 每句都回會像客服機器人；完全不回又像壞掉 |
| 告知 | `follow` 事件發一次隱私告知（比照進群告知，措辭改個人） | 個資法告知義務 |
| 退出 | `unfollow`（封鎖）→ 標 `left_at`；資料依隱私政策 60 天後刪 | 與群組一致 |
| 工時 | 約 0.5 人日（connector 事件、ingest 分流、LIFF 清單置頂、告知文案） | |

**不做**：1:1 語音／圖片以外的功能擴充、跨個人筆記與群組的自動關聯（等 projects 實體）。
