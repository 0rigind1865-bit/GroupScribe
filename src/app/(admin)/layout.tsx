...
import { GroupSwitcher, type GroupOption } from '../group-switcher';
...
export default async function AdminLayout({ children }: { children: ReactNode }) {
  ...
  if (dbConfigured()) {
    const db = getDb();
    const [{ data }, ev, tk, nt] = await Promise.all([
      db.from('groups_view').select('group_id, name, category, picture_url').order('last_at', { ascending: false }),
